...
async def run_pipeline(ticket_data: dict[str, Any], correlation_id: str | None = None) -> dict[str, Any]:
    ...
    # -- priority
    priority, priority_ms = await _run_stage("priority_predictor", priority_predictor.run, ticket_data, classification, ml_signals)
    ...
    # -- escalation
    escalation, escalation_ms = await _run_stage("escalation_detector", escalation_detector.run, ticket_data, classification, ml_signals)
    ...
    # -- response
    response, response_ms = await _run_stage("response_suggester", response_suggester.run, ticket_data, classification, escalation)
    ...
    # -- routing
    routing, routing_ms = await _run_stage("auto_router", auto_router.run, ticket_data, classification, escalation)
